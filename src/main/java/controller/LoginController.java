...
/*
 * localhost/model2/login.hta 요청을 처리하는 컨트롤러
 */
public class LoginController implements Controller{
	
	@Override
	public String process(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("LoginController의 procexx(request, response) 실행됨");
		
		// 요청객체에서 요청 파라미터값(id, password) 조회하기
		...
		if("Yes".equals(customer.getDisabled())) {
			return "redirect:loginform.hta?errir=disabled";			
		}
		// 비밀번호가 일치하는지 확인하기
